<?php
/**
 * WPBC BFB Pack: Duration Time (single duration points).
 *
 * - Adds a "durationtime" field that renders dropdown options like:
 *      [selectbox durationtime "00:15" "00:30" "00:45" "01:00" ... ]
 * - Labels can be AM/PM or 24h; values are always strict "HH:MM" (24h).
 * - Inspector UI mirrors Start Time to reuse its JS delegation:
 *      - AM/PM </> 24H switch (label/display format)
 *      - Generator: Min/Max + Step -> builds duration points
 ...
 * - Inspector root uses `.wpbc_bfb__inspector_starttime` to reuse Start Time events.
 * - Hidden locked "name" is preset to "durationtime".
 * - Preview select keeps `.wpbc_bfb__preview-timepicker` to reuse init.
 *
 * @package   Booking Calendar
 * @author    wpdevelop
 * @since     11.0.0
 * @modified  04.11.2025 13:05
 * @version   1.0.2
 */
...
/**
 * Register the "durationtime" field pack.
 *
 * @param array $packs Packs map.
 * @return array
 */
function wpbc_bfb_register_field_packs__field_durationtime_wptpl( $packs ) {

	$packs['durationtime'] = array(
		'kind'      => 'field',
		'type'      => 'durationtime',
		...
		'schema'    => array(
			'props' => array(
				'label'            => array( 'type' => 'string',  'default' => __( 'Duration time', 'booking' ) ),
				'name'             => array( 'type' => 'string',  'default' => 'durationtime' ),
				...
				'placeholder'      => array( 'type' => 'string',  'default' => '--- ' . __( 'Select duration time', 'booking' ) . ' ---' ),
				'value_differs'    => array( 'type' => 'boolean', 'default' => true ),

				// Default options: 15m -> 2h.
				'options'          => array(
					'type'    => 'array',
					'default' => array(
						array( 'label' => '00:15', 'value' => '00:15', 'selected' => false ),
						array( 'label' => '00:30', 'value' => '00:30', 'selected' => false ),
						array( 'label' => '00:45', 'value' => '00:45', 'selected' => false ),
						array( 'label' => '01:00', 'value' => '01:00', 'selected' => false ),
						array( 'label' => '01:30', 'value' => '01:30', 'selected' => false ),
						array( 'label' => '02:00', 'value' => '02:00', 'selected' => false ),
					),
				),

				// Generator defaults — 24H labels by default for durations.
				'gen_label_fmt'    => array( 'type' => 'string', 'default' => '24h' ), // 'ampm' | '24h'
				'gen_start_24h'    => array( 'type' => 'string', 'default' => '00:15' ), // min duration
				'gen_end_24h'      => array( 'type' => 'string', 'default' => '06:00' ), // max duration
				'gen_start_ampm_t' => array( 'type' => 'string', 'default' => '00:15' ),
				'gen_end_ampm_t'   => array( 'type' => 'string', 'default' => '06:00' ),
				'gen_step_h'       => array( 'type' => 'number', 'default' => 0,  'min' => 0,  'max' => 12 ),
				'gen_step_m'       => array( 'type' => 'number', 'default' => 15, 'min' => 0,  'max' => 59 ),

				'min_width'        => array( 'type' => 'string', 'default' => '180px' ),
			),
		),

		'templates_printer' => 'wpbc_bfb_field_durationtime_wptpl_print_templates',
	);

	return $packs;
}
add_filter( 'wpbc_bfb_register_field_packs', 'wpbc_bfb_register_field_packs__field_durationtime_wptpl' );

/**
 * Palette item defaults.
 *
 * @param string $group    Group key.
 * @param string $position Position key.
 * @return void
 */
function wpbc_bfb_palette_register_items__durationtime_wptpl( $group, $position ) {
	...
	<li class="wpbc_bfb__field"
		...
		data-label="<?php echo esc_attr( __( 'Duration time', 'booking' ) ); ?>"
		...
		<span class="wpbc_bfb__field-label"><?php echo esc_html( __( 'Duration time', 'booking' ) ); ?></span>
		<span class="wpbc_bfb__field-type">durationtime</span>
	</li>
	<li style="width:100%;margin:-5px 0;"><hr /></li>
	<?php
}
add_action( 'wpbc_bfb_palette_register_items', 'wpbc_bfb_palette_register_items__durationtime_wptpl', 10, 2 );

/**
 * Enqueue the Duration Time pack JS.
 *
 * Reuses Start Time pack event delegation via dependency to avoid code duplication.
 *
 * @param string $page Current admin page slug.
 * @return void
 */
function wpbc_bfb_enqueue__field_durationtime_wptpl_js( $page ) {
	wp_enqueue_script(
		'wpbc-bfb_field_durationtime_wptpl',
		wpbc_plugin_url( '/includes/page-form-builder/field-packs/time-duration/_out/field-durationtime-wptpl.js' ),
		array( 'wp-util', 'wpbc-bfb', 'wpbc-bfb-time-utils', 'wpbc-bfb_field_starttime_wptpl' ),
		WP_BK_VERSION_NUM,
		true
	);
}
add_action( 'wpbc_enqueue_js_field_pack', 'wpbc_bfb_enqueue__field_durationtime_wptpl_js', 10, 1 );

/**
 * Print wp.template() blocks (Preview + Inspector) for Duration Time.
 *
 * Markup is intentionally mirrored from Start Time to share existing JS behaviors.
 *
 * @param string $page Builder page slug.
 * @return void
 */
function wpbc_bfb_field_durationtime_wptpl_print_templates( $page ) {
	...
	?>
	<script type="text/html" id="tmpl-wpbc-bfb-field-durationtime">
		<span class="wpbc_bfb__noaction wpbc_bfb__no-drag-zone" inert="" style="min-width: {{ data.min_width || '180px' }}">
			...
			<span class="wpbc_wrap_text wpdev-form-control-wrap">
				<select
					class="wpbc_bfb__preview-input wpbc_bfb__preview-select wpbc_bfb__preview-timepicker wpbc_bfb__preview-starttime {{ data.cssclass || '' }}"
					name="{{ (data.name || data.id || 'durationtime') }}"
	...
	<script type="text/html" id="tmpl-wpbc-bfb-inspector-durationtime">
		<style type="text/css">
		...
		<!-- IMPORTANT: Reuse the same root class `.wpbc_bfb__inspector_starttime` so Start Time JS binds here too. -->
		<div class="wpbc_bfb__inspector_starttime wpbc_bfb__inspector_timepicker" data-type="durationtime"
			...
			data-i18n-rowlabel="<?php echo esc_attr__( 'Label (e.g. 01:30)', 'booking' ); ?>"
		>
			<div class="wpbc_bfb__inspector__head">
				<div class="header_container">
					<div class="header_title_content">
						<h3 class="title"><?php echo esc_html__( 'Duration time', 'booking' ); ?></h3>
						<div class="desc"><?php echo esc_html__( 'Use the generator below to create a list of duration options.', 'booking' ); ?></div>
					</div>
			...
			<div class="wpbc_bfb__inspector__body">
					...
					<div class="group__fields">
						<div class="inspector__row">
							...
							<div class="inspector__control">
								<input type="text" class="inspector__input" data-inspector-key="label" value="{{ data.label || '<?php echo esc_js( __( 'Duration time', 'booking' ) ); ?>' }}">
							</div>
						</div>
						...
						<div class="inspector__row">
							...
							<div class="inspector__control">
								<input type="text" class="inspector__input" value="{{ (data.name && data.name !== '' ? data.name : 'durationtime') }}" disabled="disabled" aria-disabled="true">
								<input type="hidden" class="inspector__input js-locked-name" data-inspector-key="name" data-locked="1" value="{{ (data.name && data.name !== '' ? data.name : 'durationtime') }}">
				...
				<section class="wpbc_bfb__inspector__group wpbc_ui__collapsible_group is-open" data-group="options">
					<button type="button" class="group__header">
						<h3><?php echo esc_html__( 'Duration options', 'booking' ); ?></h3>
						<i class="wpbc_ui_el__vert_menu_root_section_icon menu_icon icon-1x wpbc-bi-chevron-right"></i>
					</button>
					...
					<div class="group__fields inspector__group__fields__times">
						...
						<div class="inspector__row">
							<div class="inspector__control inspector__control__generator_times">
								...
								<# var __fmt_name = 'durationtime_label_fmt_' + ( data.html_id ? data.html_id : ( 'id_' + String( Math.random() ).slice(2) ) ); #>
								<h2><?php echo esc_html__( 'Generate durations', 'booking' ); ?></h2>
								<div class="wpbc_bfb__st_gen_format wpbc_inline_inputs" role="radiogroup" aria-label="<?php echo esc_attr__( 'Display format for labels', 'booking' ); ?>" style="display:none;">
									<label class="inspector__label inspector__w_30"><?php echo esc_html__( 'Label format', 'booking' ); ?>:</label>
									<div class="wpbc_inline_inputs inspector__w_70">

										<input type="hidden" class="inspector__input js-st-label-fmt-value" data-inspector-key="gen_label_fmt" value="{{ data.gen_label_fmt || '24h' }}">

										<label>
											<input type="radio" name="{{ __fmt_name }}" class="js-st-label-fmt" value="ampm" <# if ( (data.gen_label_fmt||'24h') === 'ampm' ) { #> checked <# } #> >
											<?php echo esc_html__( 'AM/PM', 'booking' ); ?>
										</label>
										&nbsp;&nbsp;
										<label>
											<input type="radio" name="{{ __fmt_name }}" class="js-st-label-fmt" value="24h" <# if ( (data.gen_label_fmt||'24h') === '24h' ) { #> checked <# } #> >
								...
								<div class="wpbc_bfb__st_gen_times" style="margin-top:8px;">
									...
									<div class="js-st-24h wpbc_inline_inputs" <# if ( (data.gen_label_fmt||'24h') !== '24h' ) { #> style="display:none" hidden aria-hidden="true" <# } #>>
										<label class="inspector__label inspector__w_30"><?php echo esc_html__( 'Min / Max duration', 'booking' ); ?>:</label>
										<div class="wpbc_inline_inputs inspector__w_70">
											<div class="inspector__control inspector__w_45">
												<input type="text" class="inspector__input inspector__w_25 js-st-mask js-gen-start-24h" data-mask-kind="24h" placeholder="HH:MM" data-inspector-key="gen_start_24h" value="{{ data.gen_start_24h || '00:15' }}">
												<label class="inspector__label inspector__w_30"><?php echo esc_html__( 'hour : minutes', 'booking' ); ?></label>
											</div>
											...
											<div class="inspector__control inspector__w_45">
												<input type="text" class="inspector__input inspector__w_25 js-st-mask js-gen-end-24h" data-mask-kind="24h" placeholder="HH:MM" data-inspector-key="gen_end_24h" value="{{ data.gen_end_24h || '06:00' }}">
												<label class="inspector__label inspector__w_30"><?php echo esc_html__( 'hour : minutes', 'booking' ); ?></label>
											</div>
										</div>
									</div>
									...
									<div class="js-st-ampm wpbc_inline_inputs" <# if ( (data.gen_label_fmt||'24h') === '24h' ) { #> style="display:none" hidden aria-hidden="true" <# } #>>
										...
										<div class="wpbc_inline_inputs inspector__w_70">
											<div class="inspector__control inspector__w_45">
												<input type="time" class="inspector__input inspector__w_25 js-gen-start-ampm-time" step="300" data-inspector-key="gen_start_ampm_t" value="{{ data.gen_start_ampm_t || '00:15' }}">
											</div>
											<div class="inspector__control inspector__w_10" style="text-align: center;font-size: 18px;line-height: 2;font-weight: 700;">&ndash;</div>
											<div class="inspector__control inspector__w_45">
												<input type="time" class="inspector__input inspector__w_25 js-gen-end-ampm-time" step="300"  data-inspector-key="gen_end_ampm_t" value="{{ data.gen_end_ampm_t || '06:00' }}">
											</div>
										</div>
									</div>
								</div>
								...
								<div class="wpbc_bfb__st_gen_step wpbc_inline_inputs">
									<label class="inspector__label inspector__w_30"><?php echo esc_html__( 'Step between options', 'booking' ); ?></label>
								...
								<div class="wpbc_bfb__st_gen_step wpbc_inline_inputs">
									...
									<div class="wpbc_len_group wpbc_inline_inputs inspector__w_70" data-len-group="st-step-minutes">
										...
										<input data-len-range type="range"  class="inspector__input js-gen-step-m-range" min="0" max="59" step="5" value="{{ data.gen_step_m || 15 }}">
										<input data-len-value type="number" class="inspector__input inspector__w_30 js-gen-step-m" data-inspector-key="gen_step_m" min="0" max="59" step="5" value="{{ data.gen_step_m || 15 }}">
									</div>
								</div>
								...
								<div class="wpbc_bfb__options_toolbar wpbc_inline_inputs">
									<div class="inspector__control inspector__w_100" style="text-align: center;">
										<button type="button" class="button button-primary js-st-generate"><?php echo esc_html__( 'Generate durations', 'booking' ); ?> <i class="menu_icon icon-1x wpbc_icn_arrow_downward" aria-hidden="true"></i></button>
									</div>
								</div>
						...
						<div class="inspector__row  wpbc_bfb__options_editor">
							<div class="wpbc_bfb__options_list__header">
								...
								<div class="wpbc_bfb__header_col__value"><?php echo esc_html__( 'Duration', 'booking' ) . ' | ' . esc_html__( '24H', 'booking' ); ?></div>
							...
							<div class="wpbc_bfb__options_list">
								...
								<# _opts2.forEach( function( opt, idx ){
									...
									var __uid  = 'wpbc_ins_durationtime_' + Math.random().toString(36).slice(2,10);
								...
								<div class="wpbc_bfb__options_row" data-index="{{ idx }}">
									...
									<input type="text" class="wpbc_bfb__opt-label" placeholder="<?php echo esc_attr__( 'Label (e.g. 1h 30m)', 'booking' ); ?>" value="{{ opt && opt.label || '' }}">
								...
								<div class="wpbc_bfb__options_toolbar wpbc_inline_inputs">
									...
									<div class="inspector__control inspector__w_85">
										<button type="button" class="button button-secondary js-st-add-option">
											...
											<?php echo esc_html__( 'Add duration', 'booking' ); ?>
										</button>
									</div>
								</div>
							</div>
						...
						<div class="inspector__row js-placeholder-row">
							...
							<div class="inspector__control">
								<input type="text" class="inspector__input js-placeholder" data-inspector-key="placeholder" value="{{ data.placeholder || '<?php echo '--- ' . esc_js( __( 'Select duration time', 'booking' ) ) . ' ---'; ?>' }}">
