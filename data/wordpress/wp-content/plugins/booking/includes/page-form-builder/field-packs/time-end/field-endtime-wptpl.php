<?php
/**
 * WPBC BFB Pack: End Time (single time points).
 *
 * - Adds an "endtime" field that renders dropdown options like:
 *      [selectbox endtime "08:00" "09:00" ... ]
...
/**
 * Register the "endtime" field pack.
 *
 * @param array $packs Packs map.
 * @return array
 */
function wpbc_bfb_register_field_packs__field_endtime_wptpl( $packs ) {
	$packs['endtime'] = array(
		'kind'      => 'field',
		'type'      => 'endtime',
		...
		'schema'    => array(
			'props' => array(
				'label'            => array( 'type' => 'string',  'default' => __( 'End time', 'booking' ) ),
				'name'             => array( 'type' => 'string',  'default' => 'endtime' ),
		...
		'templates_printer' => 'wpbc_bfb_field_endtime_wptpl_print_templates',
	);

	return $packs;
}
add_filter( 'wpbc_bfb_register_field_packs', 'wpbc_bfb_register_field_packs__field_endtime_wptpl' );
 ...
 * @return void
 */
function wpbc_bfb_palette_register_items__endtime_wptpl( $group, $position ) {
	...
	<li class="wpbc_bfb__field"
		data-id="endtime"
		data-type="endtime"
		data-usage_key="endtime"
		data-usagenumber="1"
		data-min_width="180px"
		data-label="<?php echo esc_attr( __( 'End time', 'booking' ) ); ?>"
		data-name="endtime"
		data-required="true"
		data-autoname="0"
		data-fresh="0"
		...
		<i class="menu_icon icon-1x wpbc_icn_update"></i>
		<span class="wpbc_bfb__field-label"><?php echo esc_html( __( 'End time', 'booking' ) ); ?></span>
		<span class="wpbc_bfb__field-type">endtime</span>
	</li>
	<?php
}
add_action( 'wpbc_bfb_palette_register_items', 'wpbc_bfb_palette_register_items__endtime_wptpl', 10, 2 );

/**
 * Enqueue the End Time pack JS.
 *
 * @param string $page Current admin page slug.
 * @return void
 */
function wpbc_bfb_enqueue__field_endtime_wptpl_js( $page ) {
	// iMask.js is already enqueued on this page.
	wp_enqueue_script(
		'wpbc-bfb_field_endtime_wptpl',
		wpbc_plugin_url( '/includes/page-form-builder/field-packs/time-end/_out/field-endtime-wptpl.js' ),
		array( 'wp-util', 'wpbc-bfb', 'wpbc-bfb-time-utils' ),
		WP_BK_VERSION_NUM,
		true
	);
}
add_action( 'wpbc_enqueue_js_field_pack', 'wpbc_bfb_enqueue__field_endtime_wptpl_js', 10, 1 );

/**
 * Print wp.template() blocks (Preview + Inspector) for End Time.
 *
 * @param string $page Builder page slug.
 * @return void
 */
function wpbc_bfb_field_endtime_wptpl_print_templates( $page ) {
	...
	?>
	<script type="text/html" id="tmpl-wpbc-bfb-field-endtime">
		<span class="wpbc_bfb__noaction wpbc_bfb__no-drag-zone" inert="" style="min-width: {{ data.min_width || '180px' }}">
			...
			<span class="wpbc_wrap_text wpdev-form-control-wrap">
				<select
					class="wpbc_bfb__preview-input wpbc_bfb__preview-select wpbc_bfb__preview-endtime wpbc_bfb__preview-timepicker {{ data.cssclass || '' }}"
					name="{{ (data.name || data.id || 'endtime') }}"
	...
	<script type="text/html" id="tmpl-wpbc-bfb-inspector-endtime">
		<style type="text/css">
		...
		<!--
			NOTE:
			We intentionally include the class "wpbc_bfb__inspector_starttime" here
			to **reuse** the Start Time pack's delegated JS event handlers
			(generator, options editor, masks, etc.) without duplicating code.
			This panel is still unique via "wpbc_bfb__inspector_endtime".
		-->
		<div class="wpbc_bfb__inspector_endtime wpbc_bfb__inspector_timepicker wpbc_bfb__inspector_starttime" data-type="endtime"
			...
			data-i18n-rowlabel="<?php echo esc_attr__( 'Label (e.g. 05:00 PM)', 'booking' ); ?>"
		>
			<div class="wpbc_bfb__inspector__head">
				<div class="header_container">
					<div class="header_title_content">
						<h3 class="title"><?php echo esc_html__( 'End time', 'booking' ); ?></h3>
			...
			<div class="wpbc_bfb__inspector__body">
					...
					<div class="group__fields">
						<div class="inspector__row">
							...
							<div class="inspector__control">
								<input type="text" class="inspector__input" data-inspector-key="label" value="{{ data.label || '<?php echo esc_js( __( 'End time', 'booking' ) ); ?>' }}">
							</div>
						</div>
						...
						<div class="inspector__row">
							...
							<div class="inspector__control">
								<input type="text" class="inspector__input" value="{{ (data.name && data.name !== '' ? data.name : 'endtime') }}" disabled="disabled" aria-disabled="true">
								<input type="hidden" class="inspector__input js-locked-name" data-inspector-key="name" data-locked="1" value="{{ (data.name && data.name !== '' ? data.name : 'endtime') }}">
				...
				<section class="wpbc_bfb__inspector__group wpbc_ui__collapsible_group is-open" data-group="options">
					...
					<div class="group__fields inspector__group__fields__times">
						<?php /* ===== Generator — identical to Start Time; uses same class hooks ===== */ ?>
						<div class="inspector__row">
							<div class="inspector__control inspector__control__generator_times">

								<# var __fmt_name = 'et_label_fmt_' + ( data.html_id ? data.html_id : ( 'id_' + String( Math.random() ).slice(2) ) ); #>
						...
						<div class="inspector__row  wpbc_bfb__options_editor">
							...
							<div class="wpbc_bfb__options_list">
								...
								<# _opts2.forEach( function( opt, idx ){
									...
									var __uid  = 'wpbc_ins_et_' + Math.random().toString(36).slice(2,10);
								...
								<div class="wpbc_bfb__options_row" data-index="{{ idx }}">
									...
									<input type="text" class="wpbc_bfb__opt-label" placeholder="<?php echo esc_attr__( 'Label (e.g. 05:00 PM)', 'booking' ); ?>" value="{{ opt && opt.label || '' }}">
		...
		</div><!-- .wpbc_bfb__inspector_endtime -->
	</script>
	<?php
}
