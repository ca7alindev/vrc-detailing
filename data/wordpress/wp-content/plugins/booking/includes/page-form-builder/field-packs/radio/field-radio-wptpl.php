<?php
/**
 * WPBC BFB Pack: Radio Field (WP-template–driven with DnD options editor)
 *
 * - Mirrors the Select pack UX but renders a single-choice radio group.
 * - Shares the same options editor (reorder, duplicate, add/remove).
 * - No "multiple"/"size"/"placeholder" controls for radios.
 *
 * File:  ../includes/page-form-builder/field-packs/radio/field-radio-wptpl.php
 *
...
/**
 * Register the "radio" field pack (WP-template–driven Inspector).
 *
 * @param array $packs Accumulated packs from other modules.
 *
 * @return array
 */
function wpbc_bfb_register_field_packs__field_radio_wptpl( $packs ) {

	$packs['radio'] = array(
		'kind'      => 'field',
		'type'      => 'radio',
		'label'     => __( 'Radio', 'booking' ),
		'icon'      => 'wpbc_icn_radio_button_checked',
		'usage_key' => 'radio',

		// Schema drives coercion/defaults/serialization.
		'schema'    => array(
			'props' => array(
				'label'         => array( 'type' => 'string',  'default' => __( 'Radio', 'booking' ) ),
		...
		'templates_printer' => 'wpbc_bfb_field_radio_wptpl_print_templates',
	);

	return $packs;
}

add_filter( 'wpbc_bfb_register_field_packs', 'wpbc_bfb_register_field_packs__field_radio_wptpl' );

/**
 * Enqueue the Radio (WP-template) pack JS for the Builder page.
 *
 * @param string $page Current admin page slug.
 *
 * @return void
 */
function wpbc_bfb_enqueue__field_radio_wptpl_js( $page ) {
	wp_enqueue_script(
		'wpbc-bfb_field_radio_wptpl',
		wpbc_plugin_url( '/includes/page-form-builder/field-packs/radio/_out/field-radio-wptpl.js' ),
		array( 'wp-util', 'wpbc-bfb' ),
		WP_BK_VERSION_NUM,
		true
	);
}
add_action( 'wpbc_enqueue_js_field_pack', 'wpbc_bfb_enqueue__field_radio_wptpl_js', 10, 1 );

/**
 * Print wp.template() blocks for the Radio field.
 *
 * @param string $page The current page slug.
 *
 * @return void
 */
function wpbc_bfb_field_radio_wptpl_print_templates( $page ) {

	if ( WPBC_BFB_BUILDER_PAGE_SLUG !== $page ) {
		return;
	}
	?>
	<script type="text/html" id="tmpl-wpbc-bfb-field-radio">
		<span class="wpbc_bfb__noaction wpbc_bfb__no-drag-zone" inert="">
			...
			<span class="wpbc_wrap_text wpdev-form-control-wrap {{ data.cssclass || '' }}">
				<#
				// Selected set logic (single-choice):
				...
				if ( has_marked ) {
					( data.options || [] ).forEach( function(o){
						if ( o && ( true === o.selected || 'true' === o.selected || 1 === o.selected || '1' === o.selected ) && selected_key === null ) {
							selected_key = String( o.value || '' );
						}
					} );
				} else if ( data.default_value != null && String( data.default_value ).trim() !== '' ) {
					selected_key = String( data.default_value );
				}
				...
				<div class="wpbc_bfb__radio-group" role="radiogroup" aria-required="{{ is_req ? 'true' : 'false' }}">
					<# _opts.forEach( function( opt, idx ){ if ( !opt ) { return; }
						var _val = String( opt.value || '' );
						var _lbl = String( opt.label || '' );
						var _sel = ( selected_key !== null ) && ( String( selected_key ) === _val );
						var _id  = ( data.html_id ? ( data.html_id + '_' + idx ) : ( _name + '_' + idx ) );
					#>
						<label class="wpbc_bfb__radio-item">
							<input
								type="radio"
								class="wpbc_bfb__preview-input wpbc_bfb__preview-radio"
								name="{{ _name }}"
								value="{{ _val }}"
								tabindex="-1"
								aria-disabled="true"
							...
							>
							<span class="wpbc_bfb__radio-label">{{ _lbl }}</span>
						</label>
					<# }); #>
				</div>
			</span>
	...
	<script type="text/html" id="tmpl-wpbc-bfb-inspector-radio">
		<div class="wpbc_bfb__inspector__head">
			<div class="header_container">
				<div class="header_title_content">
					<h3 class="title"><?php echo esc_html__( 'Radio', 'booking' ); ?></h3>
					<div class="desc"><?php echo esc_html__( 'Configure a single-choice radio group. Drag to reorder options.', 'booking' ); ?></div>
				</div>
		...
		<div class="wpbc_bfb__inspector__body">
				...
				<div class="group__fields">
					<div class="inspector__row">
						...
						<div class="inspector__control">
							<input type="text" class="inspector__input" data-inspector-key="label" value="{{ data.label || '<?php echo esc_js( __( 'Radio', 'booking' ) ); ?>' }}">
						</div>
					</div>
			...
			<section class="wpbc_bfb__inspector__group wpbc_ui__collapsible_group" data-group="options">
				<button type="button" class="group__header">
					<h3><?php echo esc_html__( 'Options', 'booking' ); ?></h3>
					<i class="wpbc_ui_el__vert_menu_root_section_icon menu_icon icon-1x wpbc-bi-chevron-right"></i>
				</button>
				<div class="group__fields">
					...
					<div class="wpbc_bfb__options_editor">
						...
						<div class="wpbc_bfb__options_list">
							...
							if ( ( !_opts2 || !_opts2.length ) && window.WPBC_BFB_Field_Radio && typeof window.WPBC_BFB_Field_Radio.get_defaults === 'function' ) {
								_opts2 = ( window.WPBC_BFB_Field_Radio.get_defaults().options || [] ).slice( 0 );
							}
							#>
							...
							<div class="wpbc_bfb__options_row" data-index="{{ idx }}">
								...
								<input type="text" class="wpbc_bfb__opt-label" placeholder="<?php echo esc_attr__( 'Label', 'booking' ); ?>" value="{{ opt && opt.label || '' }}">
								<input type="text" class="wpbc_bfb__opt-value" placeholder="<?php echo esc_attr__( 'Value', 'booking' ); ?>" value="{{ opt && opt.value || '' }}">
								<#
								...
								<div class="wpbc_bfb__opt-selected">
									<div class="inspector__control wpbc_ui__toggle">
										...
										<label class="wpbc_ui__toggle_label" for="{{ __uid }}">
											<?php echo esc_js( __( 'Default', 'booking' ) ); ?>
										</label>
									</div>
								</div>
						...
						<div class="wpbc_bfb__options_toolbar">
							<button type="button" class="button button-secondary js-add-option">
								<i class="wpbc_ui_el__vert_nav_icon tooltip_right_offset menu_icon icon-1x wpbc_icn_add_circle" aria-hidden="true" data-original-title="<?php echo esc_attr__( 'Add option', 'booking' ); ?>"></i>
								<?php echo esc_html__( 'Add option', 'booking' ); ?>
							</button>
					...
					<div class="inspector__row">
						<label class="inspector__label"><?php echo esc_html__( 'Value different from label', 'booking' ); ?></label>
						<div class="inspector__control">
							<input type="checkbox" class="inspector__checkbox js-value-differs" data-inspector-key="value_differs"
								<# if ( true === data.value_differs || 'true' === data.value_differs || 1 === data.value_differs || '1' === data.value_differs ) { #> checked <# } #> >
						</div>
					</div>
			...
			<div class="inspector__row">
				...
				<div class="inspector__control">
					...
					<p class="wpbc_bfb__help" style="margin-top:6px;">
						<?php echo esc_html__( 'The inline layout toggle adds/removes the "group_inline" class here.', 'booking' ); ?>
					</p>
				</div>
			</div>
			...
			<div class="inspector__row">
				...
				<div class="inspector__control wpbc_ui__toggle">
					...
					<label class="wpbc_ui__toggle_label" for="{{ __iid }}">
						<?php echo esc_html__( 'Show options inline (row)', 'booking' ); ?>
					</label>
					<p id="wpbc_bfb_help_radio_inline" class="wpbc_bfb__help" style="margin-top:6px;">
						<?php echo esc_html__( 'Enables a horizontal layout by adding the "group_inline" class.', 'booking' ); ?>
					</p>
				</div>
			</div>
	...
	<script type="text/html" id="tmpl-wpbc-bfb-inspector-radio-option-row">
		<div class="wpbc_bfb__options_row" data-index="{{ data.index || 0 }}">
			<span class="wpbc_bfb__drag-handle" title="<?php echo esc_attr__( 'Drag to reorder', 'booking' ); ?>">
				<span class="wpbc_icn_drag_indicator"></span>
			</span>
			<input type="text" class="wpbc_bfb__opt-label" placeholder="<?php echo esc_attr__( 'Label', 'booking' ); ?>" value="{{ data.label || '' }}">
			<input type="text" class="wpbc_bfb__opt-value" placeholder="<?php echo esc_attr__( 'Value', 'booking' ); ?>" value="{{ data.value || '' }}">
			...
			<div class="wpbc_bfb__opt-selected">
				<div class="inspector__control wpbc_ui__toggle">
					<input
						...
						<# if ( true === data.selected || 'true' === data.selected || 1 === data.selected || '1' === data.selected ) { #> checked <# } #>
					>
					<label class="wpbc_ui__toggle_icon_radio" for="{{ data.uid }}"></label>
					<label class="wpbc_ui__toggle_label" for="{{ data.uid }}">{{ data.toggleLabel || '<?php echo esc_js( __( 'Default', 'booking' ) ); ?>' }}</label>
				</div>
			</div>
...
/**
 * (Optional) Register the "Radio" palette item in "general/bottom".
 *
 ...
 * @return void
 */
function wpbc_bfb_palette_register_items__radio_wptpl( $group, $position ) {
	...
	<li class="wpbc_bfb__field"
		data-id="radio"
		data-type="radio"
		data-usage_key="radio"
		data-label="<?php echo esc_attr( __( 'Radio buttons', 'booking' ) ); ?>">
		<i class="menu_icon icon-1x wpbc_icn_radio_button_checked"></i>
		<span class="wpbc_bfb__field-label"><?php echo esc_html( __( 'Multi choice', 'booking' ) ); ?></span>
		<span class="wpbc_bfb__field-type">radio</span>
	</li>
	<?php
}
add_action( 'wpbc_bfb_palette_register_items', 'wpbc_bfb_palette_register_items__radio_wptpl', 10, 2 );
